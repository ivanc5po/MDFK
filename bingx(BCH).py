...
wd.get('https://www.binance.com/zh-TW/futures/BCHUSDT')
...
while True:
    try:
        ...
        if mode == 1 and bingx > price+0.2:
            closePositions()
            print("CLOSE  Balance = ", balance)
            balance = float(str(getBalance()).split(',')[5].replace('"balance":', ''))
            amount = balance*bey

        if mode == 2 and bingx < price-0.2:
            closePositions()
            print("CLOSE  Balance = ", balance)
            balance = float(str(getBalance()).split(',')[5].replace('"balance":', ''))
            amount = balance*bey

        if mode != 1 and okx < bingx-0.1:
            placeOrder(symbol=coin.upper()+"-USDT", side="Ask",volume=amount , tradeType="Market", action="Open", price=0)
            mode = 1
            price = bingx
            print("SELL", bingx)

        if mode != 2 and okx > bingx+0.1:
